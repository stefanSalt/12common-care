...
- [ ] 安全设计：SecurityConfig 白名单、JWT 校验、权限切面策略、匿名下载动态校验
- [x] 数据模型落地：表结构、索引、软删除策略、唯一性约束策略
- [ ] 文件模块：存储路径生成规则、元数据、下载鉴权分支（PUBLIC/PRIVATE）
- [ ] 通知模块：WebSocket 连接鉴权、离线存储、公告广播
- [ ] 前端设计：Pinia 权限状态、路由守卫、`v-permission` 指令、401 自动刷新流程
- [ ] 可验证性：根据 design 的“正确性属性”补齐最小测试矩阵（后续落地阶段执行）
