...
补充确认（2026-01-29，众筹后台管理补齐，开始执行）：  
12B) 删除策略：若已有捐款则禁止删除，仅允许禁用（1b）  
13A) 禁用效果：前台不展示、详情返回不存在、且禁止捐款（2a）  
14A) 管理员可编辑：title/cover/content/targetAmount/endTime/enabled（3a）  
15A) 管理员编辑后：不改变 status（4a）  

## 任务清单（执行时逐项勾选）
- [x] 数据表 + 种子权限（MySQL schema/seed + H2 test schema/data）
- [x] 后端：众筹项目审核/管理（管理员）（已补齐：详情/编辑/启用禁用/删除规则 12B）
- [x] 后端：众筹公开列表/详情（未登录可访问）
- [x] 后端：用户发布/编辑/提交审核（按确认规则）
- [x] 后端：捐款接口 + 公开捐款榜单（项目详情公开展示 latestDonations，按 6B/11A）
- [x] 后端：我的捐款（用户端）
- [x] 后端：我的众筹项目（用户端）（列表/详情，仅创建者可见）
- [x] 前端（前台）：众筹列表/详情 + 捐款 + 我的捐款
- [x] 前端（前台）：发布众筹 + 我的众筹项目（编辑/提交审核）
- [x] 前端（前台首页）：展示“最新众筹”（与轮播图/公告同页）
- [x] 前端（后台）：众筹审核/管理 + 捐款记录管理
- [x] 前端（后台）：补齐众筹项目编辑/启用禁用/删除（按 12B/13A/14A/15A）
