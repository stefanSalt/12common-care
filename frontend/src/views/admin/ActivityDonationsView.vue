...
import { listActivityDonations, type ActivityDonationDto } from '../../api/activity'

const loading = ref(false)
const rows = ref<ActivityDonationDto[]>([])
const page = reactive({ current: 1, size: 10, total: 0, activityId: '' })

async function load() {
  loading.value = true
  try {
    const data = await listActivityDonations(page.current, page.size, page.activityId.trim() || undefined)
...
const emptyText = computed(() => (loading.value ? '' : '暂无捐赠记录'))
</script>
...
<template>
  <div style="display: flex; flex-direction: column; gap: 12px">
    <el-card>
      ...
      <div style="margin-top: 8px; color: var(--el-text-color-secondary); font-size: 12px">展示所有活动的捐赠记录，可按活动ID筛选。</div>
    </el-card>
